...
public final class SsaStyle {
    ...
    public static SsaStyle fromStyleLine(String str, Format format) {
        ...
        if (split.length != format.length) {
            Log.m379w(TAG, Util.formatInvariant("Skipping malformed 'Style:' line (expected %s values, found %s): '%s'", Integer.valueOf(format.length), Integer.valueOf(split.length), str));
            return null;
        }
        try {
    ...
    public static int parseAlignment(String str) {
        ...
        Log.m379w(TAG, valueOf.length() != 0 ? "Ignoring unknown alignment: ".concat(valueOf) : new String("Ignoring unknown alignment: "));
        return -1;
    }
