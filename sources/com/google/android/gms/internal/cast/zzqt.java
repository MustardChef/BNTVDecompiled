...
public class zzqt<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    private final int zza;
    private boolean zzd;
    private volatile zzqs zze;
    private List<zzqq> zzb = Collections.emptyList();
    ...
    public final V zzk(int i) {
        ...
        if (!this.zzc.isEmpty()) {
            ...
            List<zzqq> list = this.zzb;
            Map.Entry<K, V> next = it.next();
            list.add(new zzqq(this, next.getKey(), next.getValue()));
            it.remove();
        }
        return v;
    }
    ...
    public final boolean equals(Object obj) {
        ...
        int size = size();
        if (size != zzqtVar.size()) {
            return false;
        }
        int zzc = zzc();
        if (zzc == zzqtVar.zzc()) {
            for (int i = 0; i < zzc; i++) {
                if (!zzd(i).equals(zzqtVar.zzd(i))) {
                    return false;
                }
            }
            if (zzc != size) {
                return this.zzc.equals(zzqtVar.zzc);
            }
            return true;
        }
        return entrySet().equals(zzqtVar.entrySet());
    }
    ...
    public final Iterable<Map.Entry<K, V>> zze() {
        return this.zzc.isEmpty() ? zzqp.zza() : this.zzc.entrySet();
    }
    ...
    public final V put(K k, V v) {
        ...
        int i2 = this.zza;
        if (size == i2) {
            zzqq remove = this.zzb.remove(i2 - 1);
            zzn().put((K) remove.zza(), (V) remove.getValue());
        }
        this.zzb.add(i, new zzqq(this, k, v));
        return null;
    }
}
