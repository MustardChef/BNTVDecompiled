...
public class BlockingAnalyticsEventLogger implements AnalyticsEventReceiver, AnalyticsEventLogger {
    ...
    public void logEvent(String str, Bundle bundle) {
        synchronized (this.latchLock) {
            Logger.getLogger().m151d("Logging Crashlytics event to Firebase");
            this.eventLatch = new CountDownLatch(1);
            this.callbackReceived = false;
            this.baseAnalyticsEventLogger.logEvent(str, bundle);
            Logger.getLogger().m151d("Awaiting app exception callback from FA...");
            try {
                if (this.eventLatch.await(this.timeout, this.timeUnit)) {
                    this.callbackReceived = true;
                    Logger.getLogger().m151d("App exception callback received from FA listener.");
                } else {
                    Logger.getLogger().m151d("Timeout exceeded while awaiting app exception callback from FA listener.");
                }
            } catch (InterruptedException unused) {
                Logger.getLogger().m151d("Interrupted while awaiting app exception callback from FA listener.");
            }
