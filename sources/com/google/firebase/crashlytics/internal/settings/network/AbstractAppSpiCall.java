...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData appRequestData, boolean z) {
        ...
        logger.m151d("Sending app info to " + getUrl());
        try {
            ...
            logger2.m151d(str + " app request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
            Logger logger3 = Logger.getLogger();
            logger3.m151d("Result was " + code);
            return ResponseParser.parse(code) == 0;
        } catch (IOException e) {
            Logger.getLogger().m148e("HTTP request failed.", e);
            throw new RuntimeException(e);
        }
    }

    private HttpRequest applyHeadersTo(HttpRequest httpRequest, AppRequestData appRequestData) {
        return httpRequest.header(AbstractSpiCall.HEADER_ORG_ID, appRequestData.organizationId).header(AbstractSpiCall.HEADER_GOOGLE_APP_ID, appRequestData.googleAppId).header(AbstractSpiCall.HEADER_CLIENT_TYPE, "android").header(AbstractSpiCall.HEADER_CLIENT_VERSION, this.version);
    }
