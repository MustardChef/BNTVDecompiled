...
public class DefaultSettingsSpiCall extends AbstractSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            logger.m151d("Requesting settings from " + getUrl());
            Logger logger2 = this.logger;
            logger2.m151d("Settings query params were: " + queryParamsFor);
            HttpResponse execute = applyHeadersTo.execute();
            Logger logger3 = this.logger;
            logger3.m151d("Settings request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
            return handleResponse(execute);
        } catch (IOException e) {
            this.logger.m148e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger = this.logger;
        logger.m151d("Settings result was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger2 = this.logger;
        logger2.m149e("Failed to retrieve settings from " + getUrl());
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger = this.logger;
            logger.m150d("Failed to parse settings JSON from " + getUrl(), e);
            Logger logger2 = this.logger;
            logger2.m151d("Settings response " + str);
            return null;
        }
    }
    ...
    private HttpRequest applyHeadersTo(HttpRequest httpRequest, SettingsRequest settingsRequest) {
        ...
        applyNonNullHeader(httpRequest, "Accept", AbstractSpiCall.ACCEPT_JSON_VALUE);
