...
public class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest createReportRequest, boolean z) {
        ...
        try {
            ...
            logger2.m151d("Create report request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
            Logger logger3 = Logger.getLogger();
            logger3.m151d("Result was: " + code);
            return ResponseParser.parse(code) == 0;
        } catch (IOException e) {
            Logger.getLogger().m148e("Create report HTTP request failed.", e);
            throw new RuntimeException(e);
        }
    }

    private HttpRequest applyHeadersTo(HttpRequest httpRequest, CreateReportRequest createReportRequest) {
        HttpRequest header = httpRequest.header(AbstractSpiCall.HEADER_GOOGLE_APP_ID, createReportRequest.googleAppId).header(AbstractSpiCall.HEADER_CLIENT_TYPE, "android").header(AbstractSpiCall.HEADER_CLIENT_VERSION, this.version);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Logger.getLogger().m151d("Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return part.part(FILE_PARAM, report.getFileName(), "application/octet-stream", report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Logger.getLogger().m151d("Adding file " + file.getName() + " to report " + report.getIdentifier());
