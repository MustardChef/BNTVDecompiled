...
public class CrashlyticsCore {
    ...
    public boolean onPreExecute(SettingsDataProvider settingsDataProvider) {
        ...
        logger.m151d("Mapping file ID is: " + mappingFileId);
        ...
        try {
            ...
            logger2.m147i("Initializing Crashlytics " + getVersion());
            ...
            logger3.m151d("Installer package name is: " + create.installerPackageName);
            ...
            if (didPreviousInitializationFail && CommonUtils.canTryConnection(this.context)) {
                Logger.getLogger().m151d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsDataProvider);
                return false;
            }
            Logger.getLogger().m151d("Exception handling initialization successful");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m148e("Crashlytics was not started due to an exception during initialization", e);
    ...
    public Task<Void> doBackgroundInitialization(SettingsDataProvider settingsDataProvider) {
        ...
        try {
            ...
            if (!settings.getFeaturesData().collectReports) {
                Logger.getLogger().m151d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settings.getSessionData().maxCustomExceptionEvents)) {
                Logger.getLogger().m151d("Could not finalize previous sessions.");
            }
            return this.controller.submitAllReports(1.0f, settingsDataProvider.getAppSettings());
        } catch (Exception e) {
            Logger.getLogger().m148e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    private void finishInitSynchronously(final SettingsDataProvider settingsDataProvider) {
        ...
        Logger.getLogger().m151d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            submit.get(4L, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().m148e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().m148e("Problem encountered during Crashlytics initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().m148e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    void markInitializationStarted() {
        ...
        Logger.getLogger().m151d("Initialization marker file created.");
    }
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.3
            ...
            public Boolean call() throws Exception {
                try {
                    ...
                    Logger logger = Logger.getLogger();
                    logger.m151d("Initialization marker file removed: " + remove);
                    return Boolean.valueOf(remove);
                } catch (Exception e) {
                    Logger.getLogger().m148e("Problem encountered deleting Crashlytics initialization marker.", e);
                    return false;
                }
            }
        });
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().m151d("Configured not to require a build ID.");
            return true;
