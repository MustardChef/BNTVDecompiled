...
public class CrashlyticsUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable th) {
        ...
        try {
            try {
                if (thread == 0) {
                    Logger.getLogger().m149e("Could not handle uncaught exception; null thread");
                } else if (th == null) {
                    Logger.getLogger().m149e("Could not handle uncaught exception; null throwable");
                } else {
                    this.crashListener.onUncaughtException(this.settingsDataProvider, thread, th);
                }
            } catch (Exception e) {
                Logger.getLogger().m148e("An error occurred in the uncaught exception handler", e);
            }
            Logger.getLogger().m151d("Crashlytics completed exception processing. Invoking default exception handler.");
            r1 = this.defaultHandler;
            r1.uncaughtException(thread, th);
            thread = this.isHandlingException;
            thread.set(false);
        } catch (Throwable th2) {
            Logger.getLogger().m151d(r1);
