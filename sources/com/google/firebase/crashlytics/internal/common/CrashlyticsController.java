...
public class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsDataProvider settingsDataProvider, final Thread thread, final Throwable th) {
        ...
        logger.m151d("Crashlytics is handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        ...
        try {
            Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6
                ...
                public Task<Void> call() throws Exception {
                        ...
                        if (CrashlyticsController.this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
                            ...
                            return settingsDataProvider.getAppSettings().onSuccessTask(executor, new SuccessContinuation<AppSettingsData, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6.1
                                ...
                                public Task<Void> then(AppSettingsData appSettingsData) throws Exception {
                                    ...
                                    Logger.getLogger().m143w("Received null app settings, cannot send reports at crash time.");
                    ...
                    Logger.getLogger().m149e("Tried to write a fatal exception while no session was open.");
                    return Tasks.forResult(null);
                }
            }));
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m151d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().m151d("Automatic data collection is disabled.");
        Logger.getLogger().m151d("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m151d("Waiting for send/deleteUnsentReports to be called.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m151d("Found previous crash marker.");
    ...
    public Task<Boolean> checkForUnsentReports() {
        if (!this.checkForUnsentReportsCalled.compareAndSet(false, true)) {
            Logger.getLogger().m151d("checkForUnsentReports should only be called once per execution.");
    ...
    public Task<Void> submitAllReports(float f, Task<AppSettingsData> task) {
        if (!this.reportManager.areReportsAvailable()) {
            Logger.getLogger().m151d("No reports are available.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(null);
        }
        Logger.getLogger().m151d("Unsent reports are available.");
        return waitForReportAction().onSuccessTask(new C44808(task, f));
    }

    /* renamed from: com.google.firebase.crashlytics.internal.common.CrashlyticsController$8 */
    /* loaded from: classes3.dex */
    public class C44808 implements SuccessContinuation<Boolean, Void> {
        final /* synthetic */ Task val$appSettingsDataTask;
        final /* synthetic */ float val$delay;

        C44808(Task task, float f) {
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.8.1
                ...
                public Task<Void> call() throws Exception {
                    ...
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m151d("Reports are being deleted.");
                    ...
                    Logger.getLogger().m151d("Reports are being sent.");
                    final boolean booleanValue = bool.booleanValue();
                    CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(booleanValue);
                    final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                    return C44808.this.val$appSettingsDataTask.onSuccessTask(executor, new SuccessContinuation<AppSettingsData, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.8.1.1
                        {
                            CallableC44811.this = this;
                        }

                        @Override // com.google.android.gms.tasks.SuccessContinuation
                        public Task<Void> then(AppSettingsData appSettingsData) throws Exception {
                            if (appSettingsData == null) {
                                Logger.getLogger().m143w("Received null app settings, cannot send reports during app startup.");
                                return Tasks.forResult(null);
                            }
                            ...
                            CrashlyticsController.this.reportUploaderProvider.createReportUploader(appSettingsData).uploadReportsAsync(findReports, booleanValue, C44808.this.val$delay);
    ...
    public void writeNonFatalException(final Thread thread, final Throwable th) {
        ...
        this.backgroundWorker.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.11
            ...
            public void run() {
                ...
                String currentSessionId = CrashlyticsController.this.getCurrentSessionId();
                if (currentSessionId == null) {
                    Logger.getLogger().m151d("Tried to write a non-fatal exception while no session was open.");
                    return;
                }
    ...
    public void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m149e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    private void cacheUserData(final UserMetadata userMetadata) {
        this.backgroundWorker.submit(new Callable<Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.12
            ...
            public Void call() throws Exception {
                ...
                Logger.getLogger().m151d("Tried to cache user data while no session was open.");
                return null;
            }
        });
    }
    ...
    public boolean finalizeSessions(int i) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().m151d("Skipping session finalization because a crash has already occurred.");
            Boolean bool = Boolean.FALSE;
            return false;
        }
        Logger.getLogger().m151d("Finalizing previously open sessions.");
        try {
            doCloseSessions(i, true);
            Logger.getLogger().m151d("Closed all previously open sessions");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m148e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession() throws Exception {
        ...
        Logger logger = Logger.getLogger();
        logger.m151d("Opening a new session with ID " + clsuuid);
    ...
    private void doCloseSessions(int i, boolean z) throws Exception {
        ...
        if (listSortedSessionBeginFiles.length <= z) {
            Logger.getLogger().m151d("No open sessions to be closed.");
            return;
        }
        ...
        if (this.nativeComponent.hasCrashDataForSession(sessionIdFromSessionFile)) {
            ...
            if (!this.nativeComponent.finalizeSession(sessionIdFromSessionFile)) {
                Logger logger = Logger.getLogger();
                logger.m151d("Could not finalize native session: " + sessionIdFromSessionFile);
            }
        }
    ...
    private void closeOpenSessions(File[] fileArr, int i, int i2) {
        Logger.getLogger().m151d("Closing open sessions.");
        while (i < fileArr.length) {
            File file = fileArr[i];
            String sessionIdFromSessionFile = getSessionIdFromSessionFile(file);
            Logger logger = Logger.getLogger();
            logger.m151d("Closing session: " + sessionIdFromSessionFile);
    ...
    private void closeWithoutRenamingOrLog(ClsFileOutputStream clsFileOutputStream) {
        ...
        try {
            ...
            Logger.getLogger().m148e("Error closing session file stream in the presence of an exception", e);
        }
    }
    ...
    private void retainSessions(File[] fileArr, Set<String> set) {
        for (File file : fileArr) {
            ...
            if (!matcher.matches()) {
                Logger.getLogger().m151d("Deleting unknown file: " + name);
                file.delete();
            } else if (!set.contains(matcher.group(1))) {
                Logger.getLogger().m151d("Trimming session file: " + name);
                file.delete();
            }
        }
    }
    ...
    private File[] getTrimmedNonFatalFiles(String str, File[] fileArr, int i) {
        if (fileArr.length > i) {
            Logger.getLogger().m151d(String.format(Locale.US, "Trimming down to %d logged exceptions.", Integer.valueOf(i)));
    ...
    void doCleanInvalidTempFiles(File[] fileArr) {
        ...
        for (File file : fileArr) {
            Logger.getLogger().m151d("Found invalid session part file: " + file);
        ...
        for (File file2 : listFilesMatching(new FilenameFilter() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.16
            ...
            Logger.getLogger().m151d("Deleting invalid session file: " + file2);
            file2.delete();
        }
    }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger logger = Logger.getLogger();
        logger.m151d("Finalizing native report for session " + str);
        NativeSessionFileProvider sessionFileProvider = this.nativeComponent.getSessionFileProvider(str);
        File minidumpFile = sessionFileProvider.getMinidumpFile();
        if (minidumpFile == null || !minidumpFile.exists()) {
            Logger logger2 = Logger.getLogger();
            logger2.m143w("No minidump data found for session " + str);
            return;
        }
        ...
        if (!file.mkdirs()) {
            Logger.getLogger().m151d("Couldn't create native sessions directory");
            return;
        }
    ...
    public void doWriteFatal(Thread thread, Throwable th, String str, long j) {
        ...
        try {
            ...
            try {
                ...
                e = e2;
                Logger.getLogger().m148e("An error occurred in the fatal exception logger", e);
                CommonUtils.flushOrLog(codedOutputStream, "Failed to flush to session begin file.");
                CommonUtils.closeOrLog(clsFileOutputStream, "Failed to close fatal exception file output stream.");
            }
            CommonUtils.flushOrLog(codedOutputStream, "Failed to flush to session begin file.");
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m151d("Could not write app exception marker.");
        }
    }
    ...
    public void doWriteNonFatal(Thread thread, Throwable th, String str, long j) {
        ...
        try {
            try {
                Logger.getLogger().m151d("Crashlytics is logging non-fatal exception \"" + th + "\" from thread " + thread.getName());
                ...
                try {
                    ...
                    try {
                        ...
                        crashlyticsController.writeSessionEvent(newInstance, thread, th, j, "error", false);
                        CommonUtils.flushOrLog(newInstance, "Failed to flush to non-fatal file.");
                        codedOutputStream = crashlyticsController;
                    } catch (Exception e) {
                        e = e;
                        codedOutputStream = newInstance;
                        Logger.getLogger().m148e("An error occurred in the non-fatal exception logger", e);
            ...
            try {
                ...
                Logger.getLogger().m148e("An error occurred when trimming non-fatal files.", e4);
            }
        } catch (Throwable th4) {
            th = th4;
        }
    ...
    private void writeSessionPartsToSessionFile(File file, String str, int i) {
        Logger logger = Logger.getLogger();
        logger.m151d("Collecting session parts for ID " + str);
        ...
        Logger.getLogger().m151d(String.format(Locale.US, "Session %s has fatal exception: %s", str, Boolean.valueOf(z)));
        ...
        Logger.getLogger().m151d(String.format(Locale.US, "Session %s has non-fatal exceptions: %s", str, Boolean.valueOf(z2)));
        if (z || z2) {
            synthesizeSessionFile(file, str, getTrimmedNonFatalFiles(str, listFilesMatching2, i), z ? listFilesMatching[0] : null);
        } else {
            Logger logger2 = Logger.getLogger();
            logger2.m151d("No events present for session ID " + str);
        }
        Logger logger3 = Logger.getLogger();
        logger3.m151d("Removing session part files for ID " + str);
    ...
    private void synthesizeSessionFile(File file, String str, File[] fileArr, File file2) {
        ...
        try {
            ...
            try {
                codedOutputStream = CodedOutputStream.newInstance(clsFileOutputStream);
                Logger.getLogger().m151d("Collecting SessionStart data for session ID " + str);
                ...
                e = e2;
                Logger.getLogger().m148e("Failed to write session file for session ID: " + str, e);
                CommonUtils.flushOrLog(codedOutputStream, "Error flushing session file stream");
    ...
    private static void writeNonFatalEventsTo(CodedOutputStream codedOutputStream, File[] fileArr, String str) {
        ...
        for (File file : fileArr) {
            try {
                Logger.getLogger().m151d(String.format(Locale.US, "Found Non Fatal for session ID %s in %s ", str, file.getName()));
                writeToCosFromFile(codedOutputStream, file);
            } catch (Exception e) {
                Logger.getLogger().m148e("Error writting non-fatal to session.", e);
            }
        }
    }
    ...
    private void writeInitialPartsTo(CodedOutputStream codedOutputStream, String str) throws IOException {
        ...
        for (String str2 : INITIAL_SESSION_PART_TAGS) {
            ...
            if (listFilesMatching.length == 0) {
                Logger.getLogger().m151d("Can't find " + str2 + " data for session ID " + str);
            } else {
                Logger.getLogger().m151d("Collecting " + str2 + " data for session ID " + str);
    ...
    private static void writeToCosFromFile(CodedOutputStream codedOutputStream, File file) throws IOException {
        ...
        if (!file.exists()) {
            Logger logger = Logger.getLogger();
            logger.m149e("Tried to include a file that doesn't exist: " + file.getName());
            return;
        }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m151d("Could not parse timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m151d("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
    ...
    public static final class SendReportRunnable implements Runnable {
        ...
        public void run() {
            if (CommonUtils.canTryConnection(this.context)) {
                Logger.getLogger().m151d("Attempting to send crash report at time of crash...");
