...
public class FirebaseCrashlytics {
    ...
    public static FirebaseCrashlytics init(FirebaseApp firebaseApp, FirebaseInstallationsApi firebaseInstallationsApi, CrashlyticsNativeComponent crashlyticsNativeComponent, AnalyticsConnector analyticsConnector) {
        ...
        if (analyticsConnector != null) {
            Logger.getLogger().m151d("Firebase Analytics is available.");
            ...
            if (subscribeToAnalyticsEvents(analyticsConnector, crashlyticsAnalyticsListener) != null) {
                Logger.getLogger().m151d("Firebase Analytics listener registered successfully.");
                ...
                Logger.getLogger().m151d("Firebase Analytics listener registration failed.");
            ...
            Logger.getLogger().m151d("Firebase Analytics is unavailable.");
            disabledBreadcrumbSource = new DisabledBreadcrumbSource();
        ...
        if (!onboarding.onPreExecute()) {
            Logger.getLogger().m149e("Unable to start Crashlytics.");
            return null;
        }
    ...
    private static AnalyticsConnector.AnalyticsConnectorHandle subscribeToAnalyticsEvents(AnalyticsConnector analyticsConnector, CrashlyticsAnalyticsListener crashlyticsAnalyticsListener) {
        ...
        if (registerAnalyticsConnectorListener == null) {
            Logger.getLogger().m151d("Could not register AnalyticsConnectorListener with Crashlytics origin.");
            ...
            if (registerAnalyticsConnectorListener != null) {
                Logger.getLogger().m143w("A new version of the Google Analytics for Firebase SDK is now available. For improved performance and compatibility with Crashlytics, please update to the latest version.");
            }
        }
    ...
    public void recordException(Throwable th) {
        if (th == null) {
            Logger.getLogger().m143w("Crashlytics is ignoring a request to log a null exception.");
        } else {
