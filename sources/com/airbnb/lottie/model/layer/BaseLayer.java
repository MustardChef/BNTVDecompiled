...
import com.airbnb.lottie.C1012L;
...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    public static BaseLayer forModel(Layer layer, LottieDrawable lottieDrawable, LottieComposition lottieComposition) {
        switch (C10612.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C1012L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C1012L.endSection(this.drawTraceName);
            return;
        }
        buildParentLayerListIfNeeded();
        C1012L.beginSection("Layer#parentMatrix");
        this.matrix.reset();
        this.matrix.set(matrix);
        for (int size = this.parentLayers.size() - 1; size >= 0; size--) {
            this.matrix.preConcat(this.parentLayers.get(size).transform.getMatrix());
        }
        C1012L.endSection("Layer#parentMatrix");
        ...
        if (!hasMatteOnThisLayer() && !hasMasksOnThisLayer()) {
            ...
            C1012L.beginSection("Layer#drawLayer");
            drawLayer(canvas, this.matrix, intValue);
            C1012L.endSection("Layer#drawLayer");
            recordRenderTime(C1012L.endSection(this.drawTraceName));
            return;
        }
        C1012L.beginSection("Layer#computeBounds");
        ...
        C1012L.endSection("Layer#computeBounds");
        if (!this.rect.isEmpty()) {
            C1012L.beginSection("Layer#saveLayer");
            this.contentPaint.setAlpha(255);
            Utils.saveLayerCompat(canvas, this.rect, this.contentPaint);
            C1012L.endSection("Layer#saveLayer");
            clearCanvas(canvas);
            C1012L.beginSection("Layer#drawLayer");
            drawLayer(canvas, this.matrix, intValue);
            C1012L.endSection("Layer#drawLayer");
            ...
            if (hasMatteOnThisLayer()) {
                C1012L.beginSection("Layer#drawMatte");
                C1012L.beginSection("Layer#saveLayer");
                Utils.saveLayerCompat(canvas, this.rect, this.mattePaint, 19);
                C1012L.endSection("Layer#saveLayer");
                clearCanvas(canvas);
                this.matteLayer.draw(canvas, matrix, intValue);
                C1012L.beginSection("Layer#restoreLayer");
                canvas.restore();
                C1012L.endSection("Layer#restoreLayer");
                C1012L.endSection("Layer#drawMatte");
            }
            C1012L.beginSection("Layer#restoreLayer");
            canvas.restore();
            C1012L.endSection("Layer#restoreLayer");
    ...
    private void intersectBoundsWithMask(RectF rectF, Matrix matrix) {
        ...
        if (hasMasksOnThisLayer()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                int i2 = C10612.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask.getMaskMode().ordinal()];
                if (i2 == 1 || i2 == 2) {
                    return;
                }
    ...
    /* loaded from: classes.dex */
    public static /* synthetic */ class C10612 {
