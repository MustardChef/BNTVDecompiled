...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
        ...
        synchronized (this) {
            ...
            if (this.isCancelled) {
                ...
                while (it.hasNext()) {
                    ...
                    next.executor.execute(new CallResourceReady(next.f2737cb));
                }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            if (this.isCancelled) {
                ...
                while (it.hasNext()) {
                    ResourceCallbackAndExecutor next = it.next();
                    next.executor.execute(new CallLoadFailed(next.f2737cb));
                }
                decrementPendingCallbacks();
            }
        }
    ...
    public class CallLoadFailed implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f2735cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f2735cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f2735cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f2735cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f2735cb);
                    }
    ...
    public class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f2736cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f2736cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f2736cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f2736cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f2736cb);
                        EngineJob.this.removeCallback(this.f2736cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    public static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb */
        final ResourceCallback f2737cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f2737cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f2737cb.equals(((ResourceCallbackAndExecutor) obj).f2737cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f2737cb.hashCode();
        }
    }
