...
public final class ObservableWindowBoundarySelector<T, B, V> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    static final class WindowBoundaryMainObserver<T, B, V> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        /* renamed from: ws */
        final List<UnicastSubject<T>> f10404ws;
        ...
        WindowBoundaryMainObserver(Observer<? super Observable<T>> observer, ObservableSource<B> observableSource, Function<? super B, ? extends ObservableSource<V>> function, int i) {
            ...
            this.f10404ws = new ArrayList();
            atomicLong.lazySet(1L);
        }

        @Override // io.reactivex.Observer
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f10403s, disposable)) {
                this.f10403s = disposable;
        ...
        public void onNext(T t) {
            if (fastEnter()) {
                for (UnicastSubject<T> unicastSubject : this.f10404ws) {
        ...
        void drainLoop() {
            ...
            List<UnicastSubject<T>> list = this.f10404ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (poll instanceof WindowOperation) {
                        ...
                        if (windowOperation.f10405w != null) {
                            if (list.remove(windowOperation.f10405w)) {
                                windowOperation.f10405w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    disposeBoundary();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseObserver<T, V> operatorWindowBoundaryCloseObserver) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseObserver.f10402w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    public static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f10405w;

        WindowOperation(UnicastSubject<T> unicastSubject, B b) {
            this.f10405w = unicastSubject;
            this.open = b;
        }
    }
    ...
    public static final class OperatorWindowBoundaryCloseObserver<T, V> extends DisposableObserver<V> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f10402w;

        OperatorWindowBoundaryCloseObserver(WindowBoundaryMainObserver<T, ?, V> windowBoundaryMainObserver, UnicastSubject<T> unicastSubject) {
            this.parent = windowBoundaryMainObserver;
            this.f10402w = unicastSubject;
        }
