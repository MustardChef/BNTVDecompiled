...
public abstract class FunctionDescriptorImpl extends DeclarationDescriptorNonRootImpl implements FunctionDescriptor {
    ...
    private volatile Functions<Collection<FunctionDescriptor>> lazyOverriddenFunctionsTask;
    ...
    private void performOverriddenLazyCalculationIfNeeded() {
        Functions<Collection<FunctionDescriptor>> functions = this.lazyOverriddenFunctionsTask;
        if (functions != null) {
            this.overriddenFunctions = functions.invoke();
            this.lazyOverriddenFunctionsTask = null;
        }
    }
        ...
        if (zArr[0] || !copyConfiguration.justForTypeSubstitution) {
            ...
            if (copyConfiguration.copyOverrides && !getOriginal().getOverriddenDescriptors().isEmpty()) {
                if (copyConfiguration.substitution.isEmpty()) {
                    Functions<Collection<FunctionDescriptor>> functions = this.lazyOverriddenFunctionsTask;
                    if (functions != null) {
                        createSubstitutedCopy.lazyOverriddenFunctionsTask = functions;
                    } else {
                        createSubstitutedCopy.setOverriddenDescriptors(getOverriddenDescriptors());
                    }
                } else {
                    createSubstitutedCopy.lazyOverriddenFunctionsTask = new Functions<Collection<FunctionDescriptor>>() { // from class: kotlin.reflect.jvm.internal.impl.descriptors.impl.FunctionDescriptorImpl.1
    ...
    public static List<ValueParameterDescriptor> getSubstitutedValueParameters(FunctionDescriptor functionDescriptor, List<ValueParameterDescriptor> list, TypeSubstitutor typeSubstitutor, boolean z, boolean z2, boolean[] zArr) {
        Functions<List<VariableDescriptor>> functions;
        if (list == null) {
            $$$reportNull$$$0(30);
        }
        if (typeSubstitutor == null) {
        ...
        for (ValueParameterDescriptor valueParameterDescriptor : list) {
            ...
            if (valueParameterDescriptor instanceof ValueParameterDescriptorImpl.WithDestructuringDeclaration) {
                ...
                functions = new Functions<List<VariableDescriptor>>() { // from class: kotlin.reflect.jvm.internal.impl.descriptors.impl.FunctionDescriptorImpl.2
                    @Override // kotlin.jvm.functions.Functions
