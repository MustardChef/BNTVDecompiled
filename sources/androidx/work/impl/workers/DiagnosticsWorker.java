...
public class DiagnosticsWorker extends Worker {
    ...
    private static String workSpecRows(WorkNameDao workNameDao, WorkTagDao workTagDao, SystemIdInfoDao systemIdInfoDao, List<WorkSpec> workSpecs) {
        ...
        for (WorkSpec workSpec : workSpecs) {
            Integer num = null;
            SystemIdInfo systemIdInfo = systemIdInfoDao.getSystemIdInfo(workSpec.f145id);
            if (systemIdInfo != null) {
                num = Integer.valueOf(systemIdInfo.systemId);
            }
            sb.append(workSpecRow(workSpec, TextUtils.join(",", workNameDao.getNamesForWorkSpecId(workSpec.f145id)), num, TextUtils.join(",", workTagDao.getTagsForWorkSpecId(workSpec.f145id))));
        }
        return sb.toString();
    }

    private static String workSpecRow(WorkSpec workSpec, String name, Integer systemId, String tags) {
        return String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", workSpec.f145id, workSpec.workerClassName, systemId, workSpec.state.name(), name, tags);
    }
}
